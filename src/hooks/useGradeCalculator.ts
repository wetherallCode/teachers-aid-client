...
export const useGradeCalculator = ({
	...
	if (!assignmentLoading && !responsibilityPointsLoading) {
		if (!applicableCurrentMarkingPeriodResponsiblityPoints && essays && secondaryGrades) {
			...
			const secondary = secondaryGrade(secondaryGradesEarnedPoints, secondaryGradesMaxPoints)
		...
		if (!essays && !secondaryGrades && applicableCurrentMarkingPeriodResponsiblityPoints) {
		...
		if (!applicableCurrentMarkingPeriodResponsiblityPoints && !essays && secondaryGrades) {
			...
			return {
				grade: +secondaryGrade(secondaryGradesEarnedPoints, secondaryGradesMaxPoints).toFixed(2),
				loading: assignmentLoading || responsibilityPointsLoading,
				noGrade: false,
			}
		}
		...
		if (!secondaryGrades && essays && applicableCurrentMarkingPeriodResponsiblityPoints) {
		...
		const secondaryGradeAssignments = data?.findAssignmentByStudentId.assignments.filter(
			(assignment) =>
				assignment.gradeType === GradeTypeEnum.SECONDARY &&
				assignment.markingPeriod === markingPeriod &&
				!assignment.exempt &&
