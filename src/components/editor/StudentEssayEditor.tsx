...
import { UPDATE_WORKING_DRAFT_MUTATION } from '../dashboard/student/assignments/essays/EssayToComplete'
import {
  findEssayById_findEssayById_essay,
  UpdateWorkingDraft,
  UpdateWorkingDraftVariables,
} from '../../schemaTypes'
import { useStudentEssayContextProvider } from '../dashboard/student/assignments/essays/StudentEssayContext'
import { useMutation } from '@apollo/client'
import { SubmitEssay } from '../dashboard/student/assignments/essays/SubmitEssay'
...
export const StudentEssayEditor: FC<StudentEssayEditorProps> = ({
  ...
  useEffect(() => {
    const submittedDate = new Date().toLocaleString().substring(0, 9)
    const submittedTime = new Date().toLocaleString().substring(10)

    const isLate =
      essay.dueDate >= submittedDate
        ? essay.dueTime < submittedTime
          ? true
          : false
        : false
