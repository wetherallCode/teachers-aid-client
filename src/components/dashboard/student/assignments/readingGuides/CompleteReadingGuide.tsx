...
export const CompleteReadingGuide: FC<CompleteReadingGuideProps> = ({ readingGuideInfo }) => {
	const [state, event] = useReadingGuideToCompleteContextProvider()
	const [readingGuideSteps, setReadingGuideSteps] = useState<ReadingGuideStepsTypes>('problems')

	const [problem, setProblem] = useState('')
	const [problemList, setProblemList] = useState(state.context.updateReadingGuideInputs.problems)
	...
	const [updateReadingGuide] = useMutation<updateReadingGuide, updateReadingGuideVariables>(
	...
	const multipleSections = readingGuideInfo.lessonInfo.assignedSectionIdList.length > 1
	...
	const handleDeleteProblem = (problem: string) => {
		const index = state.context.updateReadingGuideInputs.problems.findIndex((i) => i === problem)
		event({ type: 'REMOVE_PROBLEM', payload: index })
	}
	const handleDeleteImportantPeople = (person: string) => {
		const index = state.context.updateReadingGuideInputs.importantPeople.findIndex(
			(i) => i === person
		)
			...
			{state.matches('questions') && (
				<>
					{readingGuideSteps === 'problems' && (
						<ReadingGuideProblemsQuestionContainer onSubmit={(e) => e.preventDefault()}>
							<Title>
								List as many problems that are being faced in{' '}
								{multipleSections ? 'these sections' : 'this section'} as you can?
							</Title>
							<InputAndButtonContainer>
								<ReadingGuideInput autoFocus={true} onChange={(e) => setProblem(e.target.value)} />
								{problem.length > 0 ? (
									<BlueButton
										type='reset'
										onClick={() => {
											...
											event({
												...
												payload: [...state.context.updateReadingGuideInputs.problems, problem],
											})
											// setProblem('')
										}}>
										Add Problem
							...
							<ProblemsListContainer>
								{state.context.updateReadingGuideInputs.problems.length > 0 ? (
									<ProblemsLisContainerTitle>Click to Delete</ProblemsLisContainerTitle>
								) : (
									<div></div>
								)}
								<div>
									{state.context.updateReadingGuideInputs.problems.map(
										(problem: string, i: number) => (
											<ProblemsListItem key={i} onClick={() => handleDeleteProblem(problem)!}>
												{problem}
											</ProblemsListItem>
										)
									)}
							...
							{state.context.updateReadingGuideInputs.problems.length > 0 ? (
								...
								// </ButtonContainer>
								<GreyButton>Add a problem before moving forward</GreyButton>
							)}
						</ReadingGuideProblemsQuestionContainer>
					)}
					{readingGuideSteps === 'biggestProblem' && (
						<ReadingGuideBiggestProblemContainer>
							<Title>Select the problem do you think is the biggest problem?</Title>
							<ProblemsToSelectContainer>
								{state.context.updateReadingGuideInputs.problems.map(
									(problem: string, i: number) => (
										<BiggestProblemListItem
											key={i}
											selected={state.context.updateReadingGuideInputs.biggestProblem === problem}
											onClick={() =>
												event({
													type: 'SET_READING_GUIDE_PROPERTIES',
							...
							{state.context.updateReadingGuideInputs.biggestProblem && (
								<BlueButton onClick={() => setReadingGuideSteps('reasonForBiggestProblem')}>
									Next
								</BlueButton>
							)}
						</ReadingGuideBiggestProblemContainer>
					...
					{readingGuideSteps === 'reasonForBiggestProblem' && (
						<ReadingGuideReasonForBiggestProblemContainer>
							<Title>
								Why do you think '{state.context.updateReadingGuideInputs.biggestProblem}' is the
								biggest problem?
							</Title>
							<ReadingGuideInput
								autoFocus={true}
								value={state.context.updateReadingGuideInputs.reasonForBiggestProblem}
								onChange={(e) =>
									event({
										type: 'SET_READING_GUIDE_PROPERTIES',
							...
							{state.context.updateReadingGuideInputs.reasonForBiggestProblem && (
								<SmallBlueButton onClick={() => setReadingGuideSteps('importantPeople')}>
									Next
								</SmallBlueButton>
							)}
						</ReadingGuideReasonForBiggestProblemContainer>
					...
					{readingGuideSteps === 'importantPeople' && (
						<ImportantPeopleContainer>
							...
							<ProblemsListContainer>
								{state.context.updateReadingGuideInputs.importantPeople.length > 0 ? (
									<ProblemsLisContainerTitle>Click to Delete</ProblemsLisContainerTitle>
								) : (
									<div></div>
								)}
							...
							{state.context.updateReadingGuideInputs.importantPeople.length > 0 && (
								<SmallBlueButton onClick={handleFinishImportantPeopleList}>Next</SmallBlueButton>
							)}
						</ImportantPeopleContainer>
					)}
					{readingGuideSteps === 'howArePeopleInvolvedInProblems' && (
						<HowAreImportantPeopleContectedContainer>
							<Title>
								How{' '}
								{state.context.updateReadingGuideInputs.importantPeople.length > 1
									? 'are these people'
									: 'is this person'}{' '}
								involved in the{' '}
							...
							<ReadingGuideTextArea
								autoFocus={true}
								value={state.context.updateReadingGuideInputs.howArePeopleInvolvedInProblems}
								onChange={(e) =>
									event({
										type: 'SET_READING_GUIDE_PROPERTIES',
							...
							/>
							{state.context.updateReadingGuideInputs.howArePeopleInvolvedInProblems && (
								<SmallBlueButton onClick={() => setReadingGuideSteps('sectionConsequences')}>
									Next
								</SmallBlueButton>
							)}
						</HowAreImportantPeopleContectedContainer>
					...
					{readingGuideSteps === 'sectionConsequences' && (
						<SectionConsequencesContainer>
							...
							<ReadingGuideInput
								autoFocus={true}
								value={state.context.updateReadingGuideInputs.sectionConsequences}
								onChange={(e) =>
									event({
										type: 'SET_READING_GUIDE_PROPERTIES',
