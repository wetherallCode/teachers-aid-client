...
export const DevelopingOrganizer: FC<DevelopingOrganizerProps> = ({ question }) => {
	const [state, event] = useStudentEssayContextProvider()

	const sentenceStructure = {
		subject: state.context.developingOrganizer.developingSentenceStructure.subject,
	...
	>(UPDATE_DEVELOPING_ORGANIZER_MUTATION, {
		variables: {
			input: {
				...
				developingSentenceStructure: state.context.developingOrganizer.developingSentenceStructure,
		...
		<>
			...
			<>
				{state.matches('organizers.developingOrganizer.identifications') && (
					<>
						<QuestionTypeContainer>
							<QuestionTypeQuestionStyle>What is the Question Type:</QuestionTypeQuestionStyle>
							<QuestionTypeAnswerSelectStyle
								autoFocus={true}
						...
						<PartsOfQuestionContainer>
							<PartsOfQuestionTitle>Identify the Parts of the Question:</PartsOfQuestionTitle>
							<PartContainer>
								<div>What is the Subject of the question: </div>
								<PartInput
									value={state.context.developingOrganizer.developingSentenceStructure.subject}
									onChange={(e: any) =>
										event({
							...
							<PartContainer>
								...
								<PartInput
									value={state.context.developingOrganizer.developingSentenceStructure.verb}
									onChange={(e: any) =>
										event({
				...
				{state.matches('organizers.developingOrganizer.restatement') && (
					<>
						<RestatementTitle>
							<div>Restate the question in the form of a statement with the correct ending:</div>
						</RestatementTitle>
						<RestatementInput
							autoFocus={true}
							value={state.context.developingOrganizer.restatement}
							onChange={(e: any) => event({ type: 'SET_RESTATEMENT', payload: e.target.value })}
						/>
						<RestatementOutput>
				...
				{state.matches('organizers.developingOrganizer.answer') && (
					<>
						<RestatementTitle>Answer the Question to the best of your ability.</RestatementTitle>
						<AnswerInput
							autoFocus={true}
							value={state.context.developingOrganizer.answer}
							onChange={(e: any) => event({ type: 'SET_ANSWER', payload: e.target.value })}
						/>
						<AnswerOutput>
				...
				{state.matches('organizers.developingOrganizer.conclusion') && (
					<>
						<RestatementTitle>Write your conclusion (pay attention to the hints!)</RestatementTitle>
						<RestatementInput
							autoFocus={true}
							value={state.context.developingOrganizer.conclusion}
							onChange={(e: any) => event({ type: 'SET_CONCLUSION', payload: e.target.value })}
						/>
						<RestatementOutput>
