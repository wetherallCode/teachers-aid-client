...
export const subjectPredicateGrading = ({
	...
	if (!subjectCheck.includes(noun[noun.length - 1])) {
		return {
			whatWentWrong: `"` + givenSubject + `" doesn't contain a noun or the complete noun.`,
			howToFix:
	...
	for (const word of verbPhraseSplitter) {
		if (verbPhraseCheck && subjectCheck.includes(word) && !givenSubject.includes(verb)) {
			return {
	...
	if (subjectCheck.join(' ').includes(verb) || subjectCheck.join(' ').includes(helpingVerb)) {
		return {
	...
	if (
		verbPhraseCheck ? predicateCheck[0] !== verbPhraseSplitter[0] : predicateCheck[0] !== wholeVerb
	) {
		return {
