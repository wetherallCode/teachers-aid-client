...
export const AcademicAnswerTypes: FC<AcademicAnswerTypesProps> = ({ organizer }) => {
	...
	return (
		<>
			{state.matches('organizers.academicOrganizer.answer.questionType') && (
				<>
					...
					<OrganizerControlButtonContainer>
						...
						<OrganizerControlButton
							onClick={() => {
								if (organizer.__typename === 'AcademicOrganizer' && organizer.answerType !== null)
									event({ type: 'NEXT' })
							}}>
							Next
						</OrganizerControlButton>
			...
			{state.matches('organizers.academicOrganizer.answer.problemSolution') &&
				state.context.academicOrganizer.questionType === QuestionTypeEnum.HOW_PROBLEM_SOLUTION && (
					<AcademicProblemSolution />
				)}
			{state.matches('organizers.academicOrganizer.answer.howCauseEffect') &&
				state.context.academicOrganizer.questionType === QuestionTypeEnum.HOW_CAUSE_EFFECT && (
					<AcademicHowCauseEffect />
				)}
			{state.matches('organizers.academicOrganizer.answer.whyCauseEffect') &&
				state.context.academicOrganizer.questionType === QuestionTypeEnum.WHY_CAUSE_EFFECT && (
