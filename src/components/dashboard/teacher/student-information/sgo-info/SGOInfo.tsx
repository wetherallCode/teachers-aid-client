...
export const SGOInfo = ({ studentId }: SGOInfoProps) => {
	...
	const { loading, data } = useQuery<findSGOEssaysByStudentId, findSGOEssaysByStudentIdVariables>(
	...
	const essaysToReview = data?.findSGOEssaysByStudentId.essays.filter(
		(essay) => essay.finalDraft && essay.markingPeriod !== 'FIRST' && essay.score.earnedPoints !== 0
	)!
	...
	if (!loading) {
		...
		const conclusionScoreAverage = totalConclusionScore / conclusionEntries.length
							...
							<div style={{ overflow: 'scroll', height: '40vh' }}>
								<div>
									{answerEntries.map((entry, i: number) => (
										<div key={i}>
											<div>
												Number {entry.number}: {entry.readings}
											</div>
											<div>{entry.score}</div>
										</div>
									))}
