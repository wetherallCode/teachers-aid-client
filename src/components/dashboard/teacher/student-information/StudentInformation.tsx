...
export const StudentInformation = ({}: StudentInformationProps) => {
	...
	const studentSearchList = data?.findAllStudents.students.filter((student) => {
		return (
			...
			apostopheRemover(student.firstName.trim()).substr(0, studentName.length).toLowerCase() ===
	...
	return (
		<StudentInformationContainer>
			...
			{state.context.student ? (
				...
				<StudentInformationTitlePageHeader>Student Information</StudentInformationTitlePageHeader>
			)}
		</StudentInformationContainer>
	)
}
