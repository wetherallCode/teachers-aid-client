...
export const AssignWhiteCohort: FC<AssignWhiteCohortProps> = ({
	...
	const deskNumber = (
		...
		const [deskNumber] = course.hasCourseInfo?.assignedSeats
			.filter(
				(seat) => seat.whiteCohortStudent?._id === student._id && seat.whiteCohortStudent?._id
			)
				...
				.map((student) => {
					return (
						<div key={student._id!}>
							<span
								style={
									assignedSeats?.map((seat) => seat.whiteCohortStudent?._id).includes(student._id)
										? { color: 'var(--blue)' }
										: { color: 'var(--grey)' }
								}
								onClick={() => {
										...
										.filter((student) => {
											return (
												!course.hasCourseInfo?.assignedSeats
													...
													.includes(student._id) && student.cohort === StudentCohortEnum.WHITE
											)
										})
										.map((student) => (
