...
export const AssignmentManager: FC<AssignmentManagerProps> = () => {
	...
	return (
		<>
			...
			<div>
				<div onClick={() => event({ type: 'RESPONSIBILITY_POINTS' })}>Responsibility Points</div>
				<div onClick={() => event({ type: 'ESSAYS' })}>Essays</div>
				<div onClick={() => event({ type: 'ARTICLE_REVIEW' })}>Article Reviews</div>
				<div onClick={() => event({ type: 'SECONDARY_GRADES' })}>Secondary Grades</div>
			</div>
