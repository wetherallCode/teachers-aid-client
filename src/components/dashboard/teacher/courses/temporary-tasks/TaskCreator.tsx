...
`
export const TaskCreator: FC<TaskCreatorProps> = ({ courseId, dateIssued, loading, data }) => {
	...
	const absentStudentList = data?.findTemporaryTasks.temporaryTasks.filter(
		(task) => task.taskNumber === state.context.taskNumber - 1 && !task.studentPresent
	)!

	const taskNumberList = data?.findTemporaryTasks
		.temporaryTasks!.map((task) => task.taskNumber)
		.reduce((acc: number[], i: number) => (acc.includes(i) ? [...acc] : [...acc, i]), [])!
	...
	const disablePlusSelectorSwitch =
		taskNumberList && state.context.taskToGradeNumber < taskNumberList.length - 1
