...
export const SubjectPredicateSplit = ({
	...
	return (
		<SubjectPredicateContainer>
			<DirectionsContainer>
				<div>Click on the sentence below to separate it into the subject and predicate.</div>
				<div>
					The subject will be on the right with a single underline, and the predicate will be on the
					left with double underline.
				</div>
			</DirectionsContainer>
			{point ? (
				<SentenceContainer>
					{newSentence.map((part, i) => {
						return (
							<span
								...
								{newSentence[0] === part ? part.charAt(0).toUpperCase() + part.slice(1) : part}
								{/* {part} */}
							</span>
						)
					})}
				...
				<SentenceContainer>
					{sentence.split('').map((letter, i) => {
						const letterIndex = sentence.split('').findIndex((letter) => letter[0])
				...
				<MessageContainer correct={correct}>
					{!correct ? (
						...
						<div>
							...
							<br />
							<div>
								"{newSentence[0].charAt(0).toUpperCase() + newSentence[0].slice(1)}" is the complete
								subject of the sentence because it is{' '}
								{question.nounType === 'PERSON' ? 'who' : 'what'} the sentence is about.
							</div>
							<div>
								"
								...
								{question.nounType === 'PERSON' ? 'PERSON' : 'THING'} is doing.
							</div>
							<br />
