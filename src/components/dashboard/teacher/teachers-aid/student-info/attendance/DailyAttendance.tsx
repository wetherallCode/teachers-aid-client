...
export type RemoveAbsenceType = (
	options?: MutationFunctionOptions<removeAbsence, removeAbsenceVariables> | undefined
) => void

export type RemoveLatenessType = (
	options?: MutationFunctionOptions<removeLateness, removeLatenessVariables> | undefined
) => void

export type UpdateResponsibilityPointsType = (
	options?:
		| MutationFunctionOptions<updateResponsibilityPoints, updateResponsibilityPointsVariables>
		| undefined
) => void

export const DailyAttendance = ({ student, absenceCheck }: DailyAttendanceProps) => {
	// const [absenceId, setAbsenceId] = useState<string | null>(null)
	const [markingPeriodState] = useMarkingPeriodContextProvider()
	const { currentMarkingPeriod } = markingPeriodState.context

	const [removeAbsence, { called, data }] = useMutation<removeAbsence, removeAbsenceVariables>(
	...
	const excusedLatenessId = todaysExcusedLateness.length !== 0 ? todaysExcusedLateness[0]._id! : ''
