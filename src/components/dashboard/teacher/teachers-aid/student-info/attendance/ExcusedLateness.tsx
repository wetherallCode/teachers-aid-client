...
export const ExcusedLateness = ({
  ...
  >(CREATE_EXCUSED_LATENESS_QUERY, {
    variables: {
      input: {
        ...
        dayLateExcused: todaysLocaleDate,
      },
    },
  ...
  const excusedLatenessCheck =
    student.hasExcusedLatenesses.length === 0 ||
    student.hasExcusedLatenesses.some(
      (late) => late.dayLateExcused !== todaysLocaleDate
    )
      ...
      {excusedLatenessCheck ? (
        <AttendanceButton
          ...
          onClick={() => {
            ...
            if (
              student.hasUnExcusedLatenesses.some(
                (l) => l.dayLate === todaysLocaleDate
              )
            ) {
              updateResponsibilityPoints({
                variables: {
