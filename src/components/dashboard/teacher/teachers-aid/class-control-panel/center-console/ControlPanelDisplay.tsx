...
export const ControlPanelDisplay = ({ presentStudentList }: ControlPanelDisplayProps) => {
	const [state, event] = useTeachersAidContextProvider()

	const { loading, data } = useQuery<findLessonByCourseAndDate, findLessonByCourseAndDateVariables>(
		FIND_LESSON_QUERY,
		{
			variables: {
				input: {
	...
	return (
		<>
				...
				!state.context.courseSelectVisible && (
					<>
						{data?.findLessonByCourseAndDate.lesson ? (
							<>
								<ProtocolManager
									protocols={data?.findLessonByCourseAndDate.lesson.duringActivities!}
									lesson={data?.findLessonByCourseAndDate.lesson!}
									presentStudentList={presentStudentList}
								/>
								{/* <Protocols
			...
			{state.matches('controlPanelActions.mainScreenManager')! &&
				...
				!state.context.courseSelectVisible && (
					<MainScreenManagerContainer>
						<MainScreenManager lesson={data?.findLessonByCourseAndDate.lesson!} />
					</MainScreenManagerContainer>
				)}
