...
export const CreateLesson: FC<CreateLessonProps> = () => {
	...
	const [createLesson] = useMutation<createLessonVariables>(CREATE_LESSON_MUTATION, {
		variables: {
			input: {
				...
				questionList,
				duringActivities:
					state.context.lessonType === LessonTypeEnum.INTRODUCTORY ? [] : duringActivity,
		...
		refetchQueries: ['findLessonByCourseAndDate', 'findLessonsByAssignedDate'],
	})

	return (
		<>
			<LessonPlannerButton onClick={() => createLesson()}>Create Lesson</LessonPlannerButton>
		</>
	)
}
