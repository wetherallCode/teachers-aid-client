...
export const DevelopingGradingTool: FC<DevelopingGradingToolProps> = ({ rubricEntries }) => {
	...
	const modifiedRubricSections = rubricSectionEnum.slice(2).concat(rubricSectionEnum.slice(0, 2))
	...
	const totalScore = rubricEntriesList.map((entry) => entry.score).reduce((a, b) => a + b, 0)
		...
		<>
			<RubricContainer>
				...
				<RubricSectionEnumContainer>
					<div
						onClick={(e: any) => {
							if (rubricSectionEnum.indexOf(state.context.currentRubricSection) > 0) {
