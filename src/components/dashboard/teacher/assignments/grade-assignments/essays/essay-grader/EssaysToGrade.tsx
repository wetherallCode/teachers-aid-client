...
import { markingPeriodFormatter, sortByLetter } from '../../../../../../../utils'
...
export const EssaysToGrade: FC<EssaysToGradeProps> = ({ courseId }) => {
	...
	const [essayList, setEssayList] = useState<findEssaysToGradeById_findEssaysToGradeById_essays[]>(
		[]
	)
	...
	const { loading, data } = useQuery<findEssaysToGradeById, findEssaysToGradeByIdVariables>(
	...
	const essaysSubmittedOnTime =
		essayList !== undefined &&
		essayList.filter(
			(essay) => essay.late === false && essay.markingPeriod === markingPeriodToGrade
		)
	const onTimeEssaysToGrade =
		essayList !== undefined &&
		essayList.some((essay) => !essay.late && essay.markingPeriod === markingPeriodToGrade)

	const lateEssays =
		essayList !== undefined &&
		essayList.filter((essay) => essay.late === true && essay.markingPeriod === markingPeriodToGrade)

	const lateEssaysToGrade =
		essayList !== undefined &&
		essayList.some((essay) => essay.late && essay.markingPeriod === markingPeriodToGrade)

	const index = markingPeriodEnum.findIndex((c: MarkingPeriodEnum) => c === markingPeriodToGrade)
		...
		<CourseEssaysToGradeContainer>
			...
			<MarkingPeriodSelector>
				<MarkingPeriodSelectorContainer>
					...
					<CurrentMarkingPeriodContainer>
						<div>{markingPeriodFormatter(markingPeriodToGrade)} Marking Period</div>
					</CurrentMarkingPeriodContainer>
					{index < 3 && (
						<MarkingPeriodSelectorForward
							onClick={() => setMarkingPeriodToGrade(markingPeriodEnum[index + 1])}>
							&gt;
						</MarkingPeriodSelectorForward>
					)}
				</MarkingPeriodSelectorContainer>
			...
			<EssayStatusSelector>
				...
				<EssayStatusSelect
					toGradeIndicator={resubmittedEssayList && resubmittedEssayList.length > 0}
			...
			<OrderSwitchContainer>
				<div onClick={() => event({ type: 'SET_ORDER_BY', payload: 'LAST_NAME' })}>
					Order By Student
				</div>
				<div onClick={() => event({ type: 'SET_ORDER_BY', payload: 'DATE' })}>Order By Date</div>
			</OrderSwitchContainer>
		</CourseEssaysToGradeContainer>
	)
}
