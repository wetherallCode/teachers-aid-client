...
export const CHECK_QUIZ_QUESTIONS_QUERY = gql`
	query checkQuizQuestionsForTextSections($input: CheckQuizQuestionsForTextSectionsInput!) {
...
export const CreateReadingGuide = ({
	...
	const [currentCourseInfo] = me.teachesCourses.filter((course) => course._id === courseId)
	...
	// )
	const currentCourseIndex = sortedCourses.findIndex((course) => course._id === courseId)
			...
			<ReadingGuideInformationSelectContainer>
				...
				<DateAssignContainer>
					<DateAssignItemContainer>
						...
						<DateAssignInput
							type='date'
							value={dateInputConverter(state.context.readingGuide.assignedDate)}
							onChange={(e: any) => {
								event({
				...
				<MaxPointsForReadingGuideContainer>
					<QuestionSelector>
						<MaxPointSelectorContainer>
							...
							<GeneralInput
								type='text'
								value={loading ? 'loading' : state.context.readingGuide.maxPoints}
								onChange={(e: any) =>
			...
			{!data ? (
				<SelectButtonContainer>
					...
					<SelectButton
						...
						{assignedCourseIds.includes(courseId) && state.context.readingGuide.dueDate
