...
export const sentenceCapitalizer = (sentence: string) => {
	...
	return splitSentence[0].toUpperCase() + splitSentence.slice(1).join(' ').replace(' | ', '')
}
...
export const timeAFunction = (startTime: string, endTime: string) => {
	...
	return endTimeMinutes - startTimeMinutes === 0
		...
		: endTimeSeconds - startTimeSeconds + (60 * endTimeMinutes - startTimeMinutes)
}
...
export const verbsWithDoubleConsonantsInPastTense = (verb: string) => {
	const lowerCaseVerb = verb.toLowerCase()
	if (lowerCaseVerb === 'stop') {
		return 'stopped'
	}
